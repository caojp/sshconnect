...
class SSHRun():
    ...
    def showStartInfo(self, options):
        logger.debug("[*]start cracking...")

        if self.singleMode:
            logger.debug("[*]IP= %s " % self.targetIp)
        else:
            logger.debug("[*]loaded %d targetips from %s  " % (len(self.targetIps), options.targetIpFile))
        logger.debug("[*]loaded %d usernames from  %s " % (len(self.usernames), options.usernameFile))
        logger.debug("[*]loaded %s passwords from  %s " % (len(self.passwords), options.passwordFile))
    ...
    def createConnect(self, username, password, targetIp, targetPort, timeoutTime):
        ...
        logger.debug("[*]Testing Target: %s username: %s  password: %s success: %d " % (
            targetIp, username, password, connection.status))
    ...
    def currentThreadResults(self):
        ...
        for connection in self.connections:
            ...
            if connection.status == 1:
                logger.debug("[#] TargetIp: %s " % connection.targetIp)
                logger.debug("[#] Username: %s " % connection.userName)
                logger.debug("[#] Password: %s " % connection.passWord)
                if self.outfile != " ":
                    appendLineToFile("ip: %s username: %s password: %s " % (
                        connection.targetIp, connection.userName, connection.passWord), self.outfile)
